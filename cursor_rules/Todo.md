...
-   [ ] **Infrastructure & Architecture**
    ...
    -   [x] 외부 API 키 발급 및 보안 관리 방안 수립 (Gemini API, Google Cloud STT/TTS)
    -   [ ] CI/CD 파이프라인 기본 구성 (GitHub Actions 등)

## 💻 Phase 2: 백엔드(서버) 개발 (Backend Development)

-   [x] **API - Core Scenario**
    -   [x] 시나리오 제공 API 구현 (`GET /api/scenarios/random`)
        -   [x] 요청 시 랜덤 시나리오 (텍스트, 이미지 URL, 음성 URL, 미션) 반환
    -   [x] 사용자 발화 처리 및 평가 API 구현 (`POST /api/interactions`)
        -   [x] (Input) 사용자 음성 파일 수신
        -   [x] (Process) STT API 연동: 음성 -> 텍스트 변환 (Google Cloud Speech-to-Text)
        -   [x] (Process) LLM API 연동: 텍스트 평가 (발음, 문법, TPO) 및 피드백 생성 (Gemini API)
        -   [x] (Process) LLM API 연동: AI 캐릭터의 응답 대사 생성 (Gemini API)
        -   [x] (Process) TTS API 연동: AI 응답 대사 -> 음성 파일 변환 (Google Cloud TTS)
        -   [x] (Output) 평가 결과(점수, 피드백, 모범 답안) 및 AI 응답 음성 파일 URL 반환
-   [ ] **API - Gamification**
...
-   [ ] **UI/UX - 공통**
    ...
    -   [x] Retrofit, Coroutines을 사용한 네트워크 모듈 구현
    -   [ ] Room을 사용한 로컬 DB 모듈 구현
-   [ ] **UI/UX - 핵심 기능 (시나리오)**
    -   [x] 시나리오 제시 화면 구현 (이미지, 텍스트, 미션 표시)
    -   [x] 캐릭터 음성 자동 재생 기능 구현
    -   [x] 마이크 권한 요청 및 음성 녹음 기능 구현
    -   [x] 녹음된 음성 파일을 서버로 전송하는 로직 구현
-   [ ] **UI/UX - 결과 및 피드백**
